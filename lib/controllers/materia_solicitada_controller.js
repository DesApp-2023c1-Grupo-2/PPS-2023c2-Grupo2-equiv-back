...
export const index = async (req, res) => {
  ...
  res.json({
    data: materia_solicitada.map(
      (materia_solicitada) => materia_solicitada.toJSON(),
      console.log('materia_solicitada.toJSON()')
    ),
  });
};
...
export const materia_carrera = async (req, res) => {
  console.log('Petición GET a materia_carrera ' + req.body.carrera);
  const materia_solicitada = await Materia_solicitada.findAll({
    where: { carrera: req.body.carrera },
  });
  if (materia_solicitada.length > 0) {
    res.json({
    ...
    res.status(404).json({
      message: `No se encontró carrera con nombre ${req.body.carrera}`,
    });
  }
};
